...
public class MicroNucleoFrame extends Frame implements WindowListener,
        Escribano, ParMaquinaProceso {
    ...
    protected void levantarProcesoFrame(ProcesoFrame p) {
        ...
        for (i = 0; i < procesos.length; i++) {
            if (procesos[i] == null) {
                procesos[i] = p;
                encontro = true;
                imprimeln("Ventana de proceso " + procesos[i].dameIdProceso()
                        + " iniciada.");
                break;
            }
        }
        ...
        synchronized (procesos) {
            for (int i = 0; i < procesos.length; i++) {
                if (procesos[i] != null && procesos[i].equals(pf)) {
                    imprimeln("Cerrando ventana del proceso "
                            + pf.dameIdProceso());
                    Pausador.pausa(2000);
                    pf.setVisible(false);
                    procesos[i] = null;
